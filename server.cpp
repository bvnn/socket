...
"    |__|                                 \n\n Pressione . para ver as instruções\nou qualquer outra tecla se quiser\n       jogar logo direto!\n\n\0");
    ...
    if (send(clientfd, buffer, strlen(buffer), 0)) {
        ...
        do {
            ...
            if(strcmp(buffer, ".") == 0) {
                send(clientfd, "\n Instruções \n\n O jogo consiste de uma mecãnica simples, você\n é um guerreiro e a sua amada foi raptada pelo\n Escabroso Rei dos Quadrados. Para tê-la de volta\n você terá que derrotar os monstros que permeiam \n o reino, os cavaleiros reais e o próprio Rei □.\n\n",259,0);
                send(clientfd, "\n", 1, 0);
            } 
            else {
                send(clientfd, "\n Here we go!!\n\n", 16, 0);
